...
About). CLAUDE.md §6 states the rules; this file makes them reproducible by adding
the *moves*, the *lexicon*, and **before/after examples**. Rules tell you what not
to do; examples teach the voice.
...
and LOG 001 (the first fully in-voice written post — the reference exemplar).

---

## 1. The register (say this in one breath)

A working lab notebook. Calm, precise, editorial. First person. Short sentences.
The work speaks; the writer does not sell. It reads like someone documenting what
they did and what they decided, so a stranger can follow the thinking and build
further. Never a startup landing page, never a portfolio brag.
...
- **Prefer full words over contractions** for a deliberate, measured feel ("it is",
  "do not", "I have"). Allow a natural contraction only where avoiding it sounds
  stilted, usually in direct address ("what's eating your time?").
- **UI / CTA copy is active and says what it does:** "Join the waitlist", not
  "Submit". "Step into the log", not "Learn more".

---

## 3. Signature moves (the voice's DNA)
...
## 5. Before / after (the heart of this file)
...
- BAD: "The brief does the heavy lifting — I'm just pointing at it — and that's the
  whole trick."
- GOOD: "The brief does the heavy lifting. I am just pointing at it. That is the
  whole trick."
...
## 6. Spoken script vs written post

They are cousins, not twins. Keep them distinct:

| | Video script (narrated) | Written post (the site) |
|---|---|---|
| Contractions | avoid — full words read cleaner aloud | prefer full words, natural contraction OK |
| Asides / "so, yeah" | fine, it is speech | trim; the page is tighter than the mouth |
| Stage directions, PAUSE, actions | present | never — strip them entirely |
| Takeaways | spoken beats | keep, as one-line takeaways |

A transcript pasted into a post gets cleaned to the written column: punctuated,
de-ummed, stage directions removed. It stays a transcript (spoken phrasing is fine)
...
- [ ] Title is a question (experiment) or a plain concrete noun-phrase (other logs).
...
- [ ] No word from the startup/avoid lists (§2, §4).
- [ ] Full words over contractions, except where stilted.
...
- [ ] Frontmatter valid: `web-status`, `web-title`, `web-pub-date`, plus
  `web-number` / `web-stage` / `web-tags` for the stamp. (See the template.)
