...
this site, build it, and write down every decision while the camera runs. It's
...
keeps styling in the markup, so there's no separate stylesheet to hunt through.
...
that it's an Astro project, and deploys it. No servers, no build settings to
...
## No CMS, because I already write everything down

There's no CMS, on purpose. I already write everything in Obsidian: notes, daily
...
is a big enough topic to be its own thing. That's the next series.

## The decisions, on the record

Every record in this lab keeps a decision register: the calls that were made, and
where each one stands. Here's the register for this build.
...
each node builds the foundation for the next one. It's a simple way to show what
...
On to the build. First, if you don't have it, grab VS Code, it's free. I make a
...
diff before I accept it, so I catch anything I don't want before it happens.
...
cloud. I publish to a private repository. Version control isn't a later thing.
...
Fitting. It just means there aren't any pages yet.
...
is short again. Every project needs one thing people remember. For this site it's
the mark up top, a small truss of nodes that builds upward and resolves onto
...
Vercel connects straight to the GitHub repo, sees it's an Astro project, builds
...
Online. Real. For more than twenty years, when someone asked where they could see my work,
...
the end. I'm Marcelo. This was Deadlink Labs, LOG 001. Build to understand.
