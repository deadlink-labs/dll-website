...
- **The emphasis ladder — one device, one role** (settled 2026-08-11). The direct sibling of "one size per role" below, and it was fixed for the same reason. Three tiers in prose: `==text==` is a **yellow highlight, rendered bold**, and marks *the finding*, the line a skimmer must not miss; `**bold**` is *structural emphasis*, a thesis sentence or a bullet lead-in; plain text is everything else, **including product and tool names**. Before this, bold did all three jobs at once and its usage ranged from 14 marks in LOG 001 to zero across LOG 015's 3,540 words, so it signalled nothing. Eleven of LOG 001's fourteen were term first-mentions (`**Astro**`, `**Vercel**`, `**premise**`), which are labels, not emphasis; they are now plain.
  ...
  - **Distribution is a rule, and the build cannot see it.** No two marks within ~10% of each other, none past ~85% of the post, and **at most one bold per H2 section**. Both archive extremes failed the last one: LOG 015 ran 3,300 words and nine sections with zero bold, LOG 012 ran thirteen bolds in nine sections. `npm run emphasis` ([`scripts/emphasis-report.mjs`](scripts/emphasis-report.mjs)) reports positions and flags all of it; `remark-mark.mjs` only ever sees the caps.
...
**If the work has real pictures, use them, and use as many as the story earns** (settled 2026-08-09, LOG 015). This supersedes the old rule, which read "Almost none." That rule was written when the archive was empty and every candidate image would have been decoration; it was a defense against stock photography, and it got mistaken for a house style. It is not one. A photograph of the actual thing, taken at the time, is the strongest evidence this site can carry. LOG 015 is the first post to ship photographs in the body, and it uses every one that survives, because each of them is proof the work happened.

Still banned, and this part does not move: **no stock photography, no AI-generated imagery, no 3D renders, no illustrative photograph of hardware that is not the hardware in question.** That last one is the trap. When LOG 015 needed to show a grid parabolic and no photograph of the actual dish survived, the answer was to describe it in prose and draw the geometry as a house specimen tile, not to reach for a picture of a similar antenna. An image on this site is evidence or it is a diagram. It is never an illustration.
...
**The one exception: a reference image, when the evidence is lost** (added 2026-08-10, LOG 015). Sometimes the work happened, was photographed, and the photographs are gone. A representative image of the *type* of equipment may stand in, on two conditions, both load-bearing. It is captioned as representative in the caption itself, in words a skimming reader cannot miss ("like this one", "not one of ours"). And its filename does not claim a provenance it lacks: LOG 015's is `rural-point-client-AP-24dBi-antenna-reference.jpeg`, deliberately without the `2006` the real photographs carry. What is still banned is the quiet version, where a catalog photo sits in the flow of real ones and the caption lets you assume. The point was never that stand-ins are dishonest. It is that an unlabelled one spends the credibility the real photographs earned.
...
- **Astro** (content collections) + **TypeScript** + **Tailwind CSS**. Astro's built-in schema validation and image optimization are load-bearing (see below).
  ...
  - **A hand-written inline `<script>` is in-idiom; a hydrated island is the thing to justify.** Five exist and each is small and self-contained: [Nav.astro](src/components/Nav.astro) (scroll hairline), [VideoEmbed.astro](src/components/VideoEmbed.astro) (the YouTube facade), `NetworkMark.astro` (the cover mark), the About contact form, and the homepage Products track's arrows ([index.astro](src/pages/index.astro), added 2026-09-14: the track scrolls natively without it; the script only pages by one card and dims the arrows at the ends, and there is no CSS-only way to scroll a container on click that does not also jump the page through a hash). Adding a sixth needs a reason; adding a framework needs a much better one (§3 Motion says the same about Framer Motion). The sixth is [ImageZoom.astro](src/components/ImageZoom.astro) (added 2026-09-21): click-to-zoom on every image in log posts and product pages, the hero included, via `medium-zoom` (MIT, ~3 KB, no dependencies). Specimen diagrams are inline `<svg>` and the library only takes `<img>`, and rasterising them for the zoom would lose Plex Mono, so the script walks the vector through the library's own choreography (a clone over the drawing's spot, the library's overlay and class names, the same scale transform): one look and one timing for both kinds, sharp at any size. The hero's srcset is dropped for its largest candidate at open time, so the zoom carries the sharp file. The reason: photographs are evidence and the 720px column downscales them (a portrait capture lands at ~465px); the build already ships each body image at its original pixel size, so the zoom shows the file we have and fetches nothing new. Overlay is neutral graphite `rgba(18,18,20,0.78)` with an 8px backdrop blur (the warm panel colour through the blur over warm paper read as muddy brown, 2026-09-21) and zero side margin with 24px of air above and below: the expanded image reaches the window edge sideways, and does not touch top or bottom. Two things the library lacks are added in the component: a keyboard hook (Enter/Space on a tabbable image), and fit-to-viewport (medium-zoom caps the zoom at the file's own pixel size, so a 632px screenshot in the 720px column did not grow at all; an own `naturalWidth`/`naturalHeight` property on each image lifts that cap, and the viewport is the only limit, as in PhotoSwipe or Fancybox). The header image, inline SVG specimens and the avatar are out of scope.
...
- Forms: contact, beta requests and waitlist signups all go through one Resend send action (`src/actions/index.ts` → `hello@`). Resend Audiences was the plan for waitlists and was retired 2026-09-19: at this size an email per signup is the list. SPF/DKIM records on Cloudflare DNS.
  ...
  - **One action, two forms** (2026-09-19). The Cassette Mixtapes beta-request form (`WaitlistForm.astro`, `PRIVATE BETA` branch) sends through the same `contact` action: name, email, one required question whose label is the note's `web-form-prompt`, and `source: beta:<product title>`, which the action turns into the subject `Beta request (<product>): <name>`. The card's CSS (`src/styles/contact-form.css`) and the submit logic (`src/lib/contact-form.ts`) are shared with About; a page's inline `<script>` only names its two roots and its fields. **Hazefield's waitlist went through the same action later the same day**, at Marcelo's call: same card, same three fields, `source: waitlist:<product title>`, subject `Waitlist (<product>): <name>`, its own receipt. `WaitlistForm.astro` is one markup with two sets of words; the Resend Audiences episode is retired.
  - **The honeypot is a real check now, in both forms.** Astro's form parsing turns an empty optional string into `undefined` before the action sees it (`handleFormDataGet` in `astro/dist/actions/runtime/server.js`), so `contactGuard` treats `undefined` and `''` alike as "left alone". The first version compared `typeof website !== 'string'` and rejected every honest submission for two days (`v1.02.100` to `v1.02.108`).
...
- **Folder and file are the title** (settled 2026-09-17, replacing the
  2026-09-15 "first sentence" rule, which let folders drift from titles).
  Folder = the full `web-title`, slugified: lowercase ASCII, punctuation
  dropped, spaces to hyphens. File = the full title as written, minus the
  characters a filesystem or Obsidian rejects (`: ? " / \ < > * |`) and any
  trailing period. So `web-title: "Nobody fills in the form. So I made WhatsApp
  the interface."` lives at
  `nobody-fills-in-the-form-so-i-made-whatsapp-the-interface/Nobody fills in the form. So I made WhatsApp the interface.md`.
  A title change renames both (`git mv` so history follows), keeps the old
  title as an Obsidian `aliases` entry, and repoints any `site.config.json`
  slug and any `/log/...` link in content. No exceptions, no shortening. Long
  URLs are fine; a post that can be found from any of its names is the point.
  **The URL changes with the folder** and there are no redirects (decided
  2026-09-17: the site is new). Settle the title before publishing; after the
  site has inbound links, a rename is a broken link.
...
- `web-status` → visibility gate. **Public if and only if `web-status: published`.** Anything else — `draft`, a typo, or a missing field — is invisible. An untagged note must resolve to invisible, so a forgotten tag never leaks. **A draft also never enters the build** (v1.02.103, 2026-09-17): the loader in [`src/content.config.ts`](src/content.config.ts) drops every non-published entry before it is stored, so a draft may reference images that do not exist yet (`![…](./assets/not-taken-yet.png)`) without failing `astro build`. Before this, Astro registered image imports for every `.md` in the folder regardless of status, and LOG 003's seven missing screenshots broke the whole site. A broken image link in a draft surfaces the moment the post is set to `published` — the right moment. The build test with a draft in the folder is the ordinary `npm run build`; nothing needs to be moved aside.
...
4. **Client work** — rail `02`, off-nav consulting surface: stamped list — **Crehana** · `CASE STUDY`, **Uruguay Outfitters** · `SHIPPED · 2026`, **Heat exchanger manufacturer** · `PROPOSAL · 2026`. Driven by `site.config.json` → `homepage.clientWork` (§4); each entry may link to its log case study via an optional `slug`.
   ...
   *A warm `--color-surface-2` tint plate was built and rejected on sight (2026-08-10).* The argument for it was symmetry — the light sibling of the then-dark Products band. The argument is wrong: a coloured plate mid-sheet just stains the paper. **The paper is the design.** If a band needs weight, it gets a rule, air, or position, never a background. Do not re-propose the tint. (Products itself gave up its graphite plate on 2026-09-14 — see band 6 — so no band on the page carries a background now.)
   ...
   *The lede is `Systems, operations and proposals for other businesses.` (settled 2026-09-18).* It names what the rows are, not who did them: the band's rows already carry the first person in their snippets, and a lede stacking "I've built, I've led, I've developed" (the `v1.02.100` wording) read as ego before it read as proof. One first-person clause at most in a lede; none is better. "Each one has its own log entry" was dropped as obvious: the rows are links.
...
**Throwback — the archive format** (added 2026-08-09, LOG 015 / THROWBACK 002). Twenty years of work happened before this lab had a URL, and none of it was written down. A throwback is a log entry that recovers one of those projects from memory and surviving photographs. It is an ordinary log entry in every mechanical sense (same folder, same collection, its own `web-number`) and additionally carries `web-series: THROWBACK` plus a `web-series-number` (§4), which stamps `THROWBACK / 001` on the record and drives the homepage band.
...
- **The number is permanent** and independent of `web-number`. LOG 013 is THROWBACK / 001, and neither number is derived from the other. *One exception on the record (2026-09-15, Marcelo's call): the two throwbacks swapped identities. Rural Point went from LOG 013 / THROWBACK 001 / 2026-08-09 to LOG 015 / THROWBACK 002 / 2026-08-23, and the Game Boy went the other way, every field together. Slugs and URLs did not move. This is not precedent; the rule stands.*
...
- **Hazefield** · COMING SOON — A generative drone/ambient music engine for long-form evolving soundscapes. Waitlist form (name, email, one question; through the contact action).
- **Yerba Trendline** (slug `yerba`; the reconstruction in LOG 016 is still called Yerba) · TESTING (draft until it ships) — a weekly trend indicator for TradingView, reconstructed from a closed-source reference and calibrated per asset (LOG 016 tells the story). Sold as the `.pine` **file** (decided 2026-09-13): invite-only scripts need a TradingView Premium plan and the chart runs on a free one, so the buyer pastes the file into their own Pine editor and it works on any plan. Checkout provider: **Payhip** (decided 2026-09-14; Gumroad's prohibited list names crypto products and "signals", Lemon Squeezy onboards through Stripe, which does not serve Argentina). All sales final with a will-not-compile exception. The package, the listing copy and the go-live order are in `yerba-indicator/production/store/`; the full plan, including the later invite-only + Whop path, is `yerba-indicator/yerba-kb/plans/10-selling-yerba.md`. **The reference indicator is never named anywhere on this site**, same rule as that repo, and the word "signals" is avoided on the product surfaces because it is what store policies key on. Two more facts on the product surfaces (2026-09-19): alerts on indicators are a paid TradingView feature (the free plan has none), said in `web-buy-note` and the body so "runs on a free account" stays true; and the per-asset parameter pair is never shown on a chart capture, because three captures at three pairs would give away the table. Every image on the product page is a real capture of the shipped file (framed by `yerba-indicator/tools/frame_shot.py`); the drawn mockup that preceded them is gone.
...
Then **tools he actually uses** (Claude Code, Codex, Obsidian, Jira, Looker Studio, Suno, Metabase, n8n, Vercel; the same nine, same order, as the LinkedIn About) — a mono chip row, no commentary. The build stack is already stated in the footer colophon, so it is not repeated here.
